...
module.exports = {
    ...
    createRefreshToken: (userId, clientId, apiId) => {
        let secret = secrets.apiIdDigest(apiId);
        return jwt.sign({ type: 'refresh' }, secret, tokenOptions(config.RefreshTokenExpiration, userId, clientId));
    },
    createAccessToken: (userId, clientId, apiId) => {
        let secret = secrets.apiIdDigest(apiId);
        return jwt.sign({ type: 'access' }, secret, tokenOptions(config.AccessTokenExpiration, userId, clientId));
    }
};