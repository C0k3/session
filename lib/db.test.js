...
describe('db', function() {
    ...
    describe('getUser', function() {
            ...
            this.db.getUser(profileData)
                .then(user => {
                    testHelper.check(done, () => {                    
                        ...
                        expect(params.TableName).to.equal('test-KODemoUsers');                    
                        expect(params.Key).to.deep.equal({Id: `${profileData.id}` });
                    });
                }).catch(err => done(err));
        });
        ...
        it('should reject if account_type is not defined', function(done) {
            let profileData = {};
            this.db.getUser(profileData)
                .then(user => {
                    done('did not reject');
                ...
                .catch(err => {
                    testHelper.check(done, () => {
                        expect(err).to.equal('account_type required');
                    });
                });
        });
