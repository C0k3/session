...
describe('db', function() {
    ...
    describe('#saveTokens()', function() {
        it('should save token info', function(done) {
            ...
            let clientId = '2';
            let apiId = '3';
            let rToken = token.createRefreshToken(userId, clientId, apiId, process.env.REFRESH_TOKEN_EXPIRATION);
            let aToken = token.createAccessToken(userId, clientId, apiId, process.env.ACCESS_TOKEN_EXPIRATION);
            ...
            this.db.saveTokens(rToken, aToken, userId, clientId)
                .then(() => {
                    testHelper.check(done, () => {
                        ...
                        let params = this.putSpy.args[0][0];
                        expect(params.TableName).to.equal(process.env.REFRESH_TOKEN_TABLE);
    ...
    describe('#getTokens()', function() {
        it('should get token info', function(done) {
            let rToken = token.createRefreshToken('1', '2', '3', process.env.REFRESH_TOKEN_EXPIRATION);
